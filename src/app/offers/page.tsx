        ...
        <div className="container mx-auto px-4">
          {loading ? (
            <div className="text-center py-20">
              <div className="inline-block animate-spin text-5xl sm:text-6xl mb-4">⏳</div>
              <p className="text-gray-600 font-bold text-sm sm:text-base">جاري التحميل...</p>
            </div>
          ) : offers.length === 0 ? (
            <div className="text-center py-20">
              <div className="text-7xl sm:text-9xl mb-6">🎁</div>
              <p className="text-xl sm:text-2xl text-gray-600 font-bold">لا توجد عروض حالياً</p>
            </div>
          ) : (
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 max-w-6xl mx-auto">
              {offers.map((offer) => (
                <div
                  key={offer.id}
                  className="bg-white rounded-xl p-4 sm:p-6 border-2 border-gray-200 hover:shadow-lg hover:border-blue-500 transition"
                >
                  {offer.image && (
                  ...
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <h3 className="text-xl sm:text-2xl font-bold text-gray-900">{offer.title}</h3>
                  </div>
                  ...
                  <Link
                    href={offer.link}
                    className="block text-center bg-blue-600 text-white py-2.5 rounded-lg font-bold text-sm sm:text-base hover:bg-blue-700 transition"
                  >
                    تسوق الآن
                  </Link>
                </div>
